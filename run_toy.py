    ...
    def joint_logdensity(self, use_true_decoder=None):
        def density(z, x):
            ...
            return torch.distributions.Normal(loc=self.decoder_net.alpha * (
                ...
                -1) + log_Pr + 0. * self.decoder_net.aux
...
class ToyDecoder(nn.Module):
    ...
    def forward(self, x):
        return self.alpha * (
                torch.sqrt(torch.sum(torch.pow(x, 2), dim=1, keepdim=True)) + self.beta) + torch.randn_like(
            x[:, :1]) * self.sigma + 0. * self.aux
...
if __name__ == '__main__':
    N = 10000
    sigma = 1.
    full_results = {'vae': [], 'iwae': [], 'ula': [], 'ais': [], 'flow': []}
    full_alphas = {'vae': [], 'iwae': [], 'ula': [], 'ais': [], 'flow': []}
    full_betas = {'vae': [], 'iwae': [], 'ula': [], 'ais': [], 'flow': []}
    for d in [2, 5, 10, 20, 50, 100, 150, 200, 300]:
        ...
        full_results['flow'].append(compute_discrepancy(flows_vae).cpu().item())
        full_alphas['flow'].append(get_alpha(flows_vae).cpu().item())
        full_betas['flow'].append(get_beta(flows_vae).cpu().item())

        print('AIS')
        run_trainer(ais_vae)
        full_results['ais'].append(compute_discrepancy(ais_vae).cpu().item())
        full_alphas['ais'].append(get_alpha(ais_vae).cpu().item())
        full_betas['ais'].append(get_beta(ais_vae).cpu().item())

        print('ULA')
        run_trainer(ula_vae)
        full_results['ula'].append(compute_discrepancy(ula_vae).cpu().item())
        full_alphas['ula'].append(get_alpha(ula_vae).cpu().item())
        full_betas['ula'].append(get_beta(ula_vae).cpu().item())
